...
(def #^{:private true} 
     datastore-service (DatastoreServiceFactory/getDatastoreService)) 
...
(defn create
  ([kind map’]
     (create (vary-meta map’ assoc ::kind kind)))
  ([map’]
...
(defn update [map’]
  (entity->map
   (reduce
    #(.setProperty %1 (name (first %2)) (second %2))
    (::entity (meta map’)) map’)))
...
#_(defn exec [query]
  (-> query (#(.prepare datastore-service %))
      .asIterable ((partial map entity->map))))

(defn exec [query]
  (-> datastore-service (.prepare query) .asIterable
      ((partial map entity->map))))
